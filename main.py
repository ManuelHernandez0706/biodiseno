...
class MostrarResultados(Screen):
    ...
    def eliminar_registro(self):
        ...
        check_string = '¿Estas seguro de querer eliminar el registro del paciente?'
    ...
    def on_enter(self):
        ...
        if int(self.edad)>40:
            if self.sexo == "femenino":
                self.res_edad.text = "4)Los huesos de debilitan luego de la menopausia. Asistir y realizarse chequeos constantemente."
            else:
                self.res_edad.text = "4)Con la edad, los huesos se debilitan. Asistir y realizarse chequeos constantemente."
        else:
            self.res_edad.text = "4)Mantener un cuidado y realizarse chequeos paulatinamente a temprana edad para evitar complicaciones"
...
class RegistroFinal(Screen):
    ...
    def on_enter(self):
        self.label1.text = 'DMO/T-Score Cuello Femoral:'
        self.label2.text = 'DMO/T-Score Columna Vertebral (L1 L4):'
...
class RecomendacionesPaciente(Screen):
    ...
    def on_enter(self):
        ...
        if int(edad)>40:
            self.res_edad.text = "4) A mayor edad, hay una probabilidad de padecer osteoporosis. Se recomienda realizarse un chequeo medico cada 6 meses."
        else:
            self.res_edad.text = "4) Agenda chequeos medicos anualmente y seguir con dichos analisis para garantizar una buena salud."
